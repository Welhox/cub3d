...
void	paint_wall(t_data *data, t_ray *ray, int pixel_row, t_mlx_img *img)
{
	int	start;
	int	y;
	...
	data->txt->pos = ((float)start - ((float)data->s_height / 2) + \
					((float)data->txt->height / 2)) * data->txt->step;
	while (y < (start + data->txt->wall_height))
	{
		data->txt->wall_y = (int)data->txt->pos % img->height;
		data->txt->pos += data->txt->step;
		safe_pixel(data->img->fg, pixel_row, y, \
					get_txt_color(img, floor(data->txt->wall_x * img->width), \
									data->txt->wall_y, 1));
		y++;
	}
}

void	paint_row(t_data *data, t_ray *ray, int pixel_row, t_mlx_img *img)
{
	ray->corr_dist = ray->distance * cos(data->pl->p_orientation - ray->orient);
	data->txt->wall_height = (64 / (ray->corr_dist * 64)) * ray->proj_plane;
	data->txt->step = 1.0 * (float)(img->height / \
					(float)data->txt->wall_height);
